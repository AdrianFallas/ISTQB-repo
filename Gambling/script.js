...
function renderQuestion(index) {
    ...
    document.getElementById("next-btn").textContent = index === questions.length - 1 ? "Finalizar prueba" : "Siguiente";
...
function displayResults(results) {
    ...
    questions.forEach((q, index) => {
        ...
        questionDiv.innerHTML = `
            ...
            <p class="${resultClass}">
                ${result.isCorrect ? "Correct o" : "Incorrecto"}
...
document.getElementById("next-btn").addEventListener("click", () => {
    if (currentQuestionIndex < questions.length - 1) {
        ...
        alert("¡Finalizar prueba!");
        evaluateQuiz(); // Evaluar el cuestionario al finalizar
    }
});
